...
def create_mordred_features():
    from mordred import Chi, ABCIndex, RingCount, Calculator, is_missing, descriptors
    # https://github.com/JacksonBurns/mordred-community    (pip install mordredcommunity)
    multiprocessing.freeze_support()

    smiles, methods, yields = load_yield()

    chemical_lib = smiles
    print("number of smiles", len(chemical_lib))

    chem_mol = [Chem.MolFromSmiles(c) for c in chemical_lib]
    chem_mol = [c for c in chem_mol if c is not None]
    print("number of valid smiles", len(chem_mol))
    print(f"chemicals loaded:  {datetime.datetime.now().strftime('%H:%M:%S')}")
    calc3 = Calculator(descriptors)

    result = calc3.pandas(chem_mol)
    result = drop_non_numeric_columns(result)
    result = convert_boolean_columns_to_int(result)
    result.index = smiles
    result.to_csv(f"data/features_mordred.csv", index=True)
    print(f"features calculated:  {datetime.datetime.now().strftime('%H:%M:%S')}")


def _calc_rdkit_desc(smile: str) -> dict:
    return Descriptors.CalcMolDescriptors(Chem.MolFromSmiles(smile))


def calc_rdkit_desc(smiles: list[str], multiprocess_: bool = True) -> pd.DataFrame:
    if multiprocess_:
        with Pool() as pool:
            return pd.DataFrame(pool.map(_calc_rdkit_desc, smiles))

    desc = [_calc_rdkit_desc(m) for m in smiles]
    return pd.DataFrame(desc)


def create_rdkit_features():
    smiles, methods, yields = load_yield_cat()
    print("number of smiles", len(smiles))
    result = calc_rdkit_desc(smiles, multiprocess_=False if len(smiles) < 500 else True)

    result = drop_non_numeric_columns(result)
    result.index = smiles
    result.to_csv(f"data/features_rdkit.csv", index=True)
    print(f"features calculated:  {datetime.datetime.now().strftime('%H:%M:%S')}")


def _calc_macc_key(smile: str) -> np.ndarray:
    try: 
        molec = Chem.MolFromSmiles(smile)
        if molec not None:
            return np.array(MACCSkeys.GenMACCSKeys(molec))
        
    except AttributeError:
        return np.arange()


def calc_maccs_keys(smiles: list[str], multiprocess_: bool = True) -> pd.DataFrame:
    header = ['bit' + str(i) for i in range(167)]
    if multiprocess_:
        with Pool() as pool:
            return pd.DataFrame(pool.map(_calc_macc_key, smiles), columns=header)

    desc = [_calc_macc_key(m) for m in smiles]
    return pd.DataFrame(desc, columns=header)


def create_MACCS_features():
    """
    smartsPatts={
  1:('?',0), # ISOTOPE
  ...
  166:('?',0), # Fragments  FIX: this can't be done in SMARTS
    ...
    smiles, methods, yields = load_yield_cat()
    print("number of smiles", len(smiles))
    result = calc_maccs_keys(smiles, multiprocess_=False if len(smiles) < 500 else True)

    result = drop_non_numeric_columns(result)
    result.index = smiles
    result.to_csv(f"data/features_MACCS.csv", index=True)
    print(f"features calculated:  {datetime.datetime.now().strftime('%H:%M:%S')}")


def _calc_morgan_key(smile: str) -> np.ndarray:
    fpgen = AllChem.GetMorganGenerator(radius=2)
    return np.array(fpgen.GetFingerprint(Chem.MolFromSmiles(smile)))


def calc_morgan_keys(smiles: list[str], multiprocess_: bool = True) -> pd.DataFrame:
    header = ['bit' + str(i) for i in range(2048)]
    if multiprocess_:
        with Pool() as pool:
            return pd.DataFrame(pool.map(_calc_morgan_key, smiles), columns=header)

    desc = [_calc_morgan_key(m) for m in smiles]
    return pd.DataFrame(desc, columns=header)


def create_morgan_features():
    """
    ...
    smiles, methods, yields = load_yield_cat()
    print("number of smiles", len(smiles))
    result = calc_morgan_keys(smiles, multiprocess_=False if len(smiles) < 500 else True)

    result = drop_non_numeric_columns(result)
    result.index = smiles
    result.to_csv(f"data/features_morgan.csv", index=True)
    print(f"features calculated:  {datetime.datetime.now().strftime('%H:%M:%S')}")


if __name__ == "__main__":
    # create_mordred_features()
    # create_rdkit_features()
    # create_MACCS_features()
    create_morgan_features()
