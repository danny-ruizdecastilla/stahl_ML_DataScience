...
def editComs(comFile , comDict , comType , chkName):
    ...
    CNMR = comDict["CNMR"]
    if comType == 1:
        geomTheory = f"# {geomTheory} symmetry=loose empiricaldispersion=GD3BJ int=(grid=ultrafine) opt freq=noraman\n"
        spEnergy = comDict["energy"]
        linkList = [f"--Link1--\n%nprocs=16\n%mem=48GB\n%chk={chkName}.chk\n# {spEnergy} EmpiricalDispersion=GD3 int=(grid=ultrafine) freq guess=read geom=check {solventType}\n\n SPE \n\n0 1\n" , 
                    f"\n--Link1--\n%nprocs=16\n%mem=48GB\n%chk={chkName}.chk\n# {CNMR} int=(grid=fine) nmr=giao guess=read geom=check {solventType}\n\n c13 NMR\n\n0 1\n\n" ,
                     ]

    elif comType == 2:
        geomTheory = f"# {geomTheory} EmpiricalDispersion=GD3 int=(grid=ultrafine) opt freq=noraman {solventType}\n"
        linkList = [
                    f"\n--Link1--\n%nprocs=16\n%mem=48GB\n%chk={chkName}.chk\n# {CNMR} int=(grid=ultrafine) nmr=giao guess=read geom=check {solventType}\n\n c13 NMR\n\n0 1\n\n", 
                 ]
